defmodule Day02 do
  ...
  TODO: Would be nice to make this work regardless of red/green/blue being known
  """
  def smallest_coverage(round1, round2) when is_map(round1) and is_map(round2) do
  ...
  """
  def get_smallest_bag(%Game{} = game) do
    game.rounds
  ...
  """
  def get_round_power(round) when is_map(round) do
  ...
  """
  def round_is_possible_part_a?(round) when is_map(round) do
    %{"red" => 12, "green" => 13, "blue" => 14}
    |> Enum.all?(fn {key, val} -> Map.get(round, key, 0) <= val end)
  end

  def game_is_possible_part_a?(%Game{} = game) do
    game.rounds
  ...
  """
  def parse_round(round_str) when is_binary(round_str) do
    round_str
    |> String.trim()
    |> String.split(",")
  ...
  """
  def parse_data(lines) when is_list(lines) do
